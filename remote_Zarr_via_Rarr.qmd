...
⛔ `Rarr`, like `stars` and `terra`, is not capable of reading multiple arrays from the remote store simultaneously.
...
We can enclose these steps in a single function with some flexibility for variable names and resolutions. This allows us to retrieve any band at any available resolution from the remote Zarr store.
...
Further tests should evaluate whether data retrieved via `Rarr` versus GDAL are identical in terms of spatial extent and offset.
