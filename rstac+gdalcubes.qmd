...
- 🚀 Create a raster data cube with a user-defined spatiotemporal geometry using `cube_view()` and `raster_cube()` in `gdalcubes` and compute the NDVI 


::: {.callout-note}
As you’ll notice, we’ll need to make some adjustments to the workflow in certain places since gdalcubes isn’t fully optimized yet for the EOPF Sentinel Zarr Samples Service. However, the required functionality is available, and we can expect it to be more seamless in future releases.
:::
...
```{r}	
# Using a bbox roughly for "Münsterland" region in Germany and the month June 2025.
...
s2_collection = stac_image_collection(items$features, asset_names = c("B02_10m", "B03_10m", "B04_10m", "B08_10m"), url_fun = f)
...
#install.packages("terra") #install if necessary
library(terra)
...
s2_collection2 = stac_image_collection(items2$features, asset_names = c("B02_10m", "B03_10m", "B04_10m", "B08_10m"), url_fun = f)
